...
class Pharmacophore:
    ...
    def __init__(self, name, ph_coords):
        """
        :param name:
        :param ph_coords:
        """
    ...
    def make_hash(self, m=1):
        ...
        center = center_mass(self.coords, self.nodes)
        new_c = new_coordinates(self.coords, center)
        I = inertia_tensor(new_c, self.nodes)
        eig = eigen(I)
        RS = only_right_vect(eig)
        C2 = coordinates_2(new_c, RS, m)
        CF = final_coordinates(C2, self.nodes)
        self.hash = CF[0]

    def is_match(self, phrm):
        return phrm.hash == self.hash

    def show(self, i='not hash'):
        """
        ...
        if i == 'hash':
            nodes = [i[0] for i in self.hash]
            coords = np.array([list(i[1]) for i in self.hash])
        else:
            nodes = self.nodes
            coords = self.coords
